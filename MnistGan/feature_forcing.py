...
class FFGAN():
  ...
  def make_encoder(self, img, encoding_size):
    ...
    net = slim.conv2d(img, 32, [5, 5], scope="conv1_11x11")
    net = slim.max_pool2d(net, [2, 2], stride=4, scope='pool1')
    net = slim.conv2d(net, 32, [3, 3], scope="conv3_5x5")
    net = slim.max_pool2d(net, [2, 2], scope='pool2')
    net = slim.conv2d(net, 32, [3, 3], scope="conv5_3x3")
    net = slim.max_pool2d(net, [2, 2], scope='pool3')
    net = slim.conv2d(net, 16, [1, 1], scope="conv6_1x1")
  ...
  def make_decoder_generator(self, encoding):
    ...
    net = slim.fully_connected(encoding, 64)
    net = tf.reshape(net, [-1, 8, 8, 1])
    net = slim.conv2d_transpose(net, 16, [1, 1], scope="conv1_3x3")
    net = tf.image.resize_images(net, [int(i) * 2 for i in net.shape[1:3]],
        tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    net = slim.conv2d_transpose(net, 32, [3, 3], scope="conv3_3x3")
    net = tf.image.resize_images(net, [28 for i in net.shape[1:3]],
        tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    net = slim.conv2d_transpose(net, 1, [1, 1], scope="conv5_5x5",
        activation_fn=None)
    return net