...
class EyeConvnet():
  ...
  def __init__(self, is_training, face_tensor,
    ...
    # share weights.
    eye_branch_template = tf.make_template(
        "eye_branch", self.make_eye_convnet)
    ...
    with slim.arg_scope([slim.fully_connected]
      ...
      with slim.arg_scope([slim.dropout],
        ...
        # No activation fn for prediction (as prediction can have
        # negative vals).
  ...
  def make_eye_convnet(self, image_input):
    ...
    with tf.variable_scope("eye_convnet"):
      with slim.arg_scope([slim.conv2d]
        ...
        net = slim.conv2d(net, 64, [3, 3], scope="conv3_3x3")
        net = slim.max_pool2d(net, [2, 2], scope='pool2')
        net = slim.conv2d(net, 32, [1, 1], scope="conv4_3x3")
        net = slim.max_pool2d(net, [2, 2], scope='pool2')
        return net
